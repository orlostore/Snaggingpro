...
function dlpGuidancePage(): string {
  return `
    <section class="page avoid-break">
      ...
      <p>Marsad Field recommends a <strong>follow-up inspection at month 11</strong> of the DLP — late enough to capture defects that develop with seasonal cycles, early enough to submit them before the warranty expires.</p>
    </section>
  `;
}
...
function inspectorPage(): string {
  return `
    <section class="page avoid-break">
      ...
      <p class="meta">Marsad Field is operated by a single qualified inspector — every report is signed in person.</p>
...
<head>
  <meta charset="utf-8">
  <title>${h(state.job.ref)} — Marsad Field Report</title>
...
<body>
  ...
  <p style="text-align:center;padding:20px;color:#999;font-size:12px">
    Generated by Marsad Field · ${h(formatDateLong(state.job.date))} · ${snags.length} snags · ${formatAED(state.property.price)} fee
  </p>
</body>
</html>`;
}
